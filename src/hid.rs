...
pub async fn hid_task(
    ...
    loop {
        ...
        match key_event.key {
            Key::EncoderLeft => {
                (keyboard_class, multimedia_class) = handle_encoder_interaction(
                    keyboard_class,
                    multimedia_class,
                    KEYLAYOUT.encoder_left,
                )
                .await;
            }
            Key::EncoderRight => {
...
async fn handle_encoder_interaction(
    ...
    match code {
        KeyType::Media(media_key) => {
            ...
            report = MediaKeyboardReport { usage_id: 0x00_u16 };
